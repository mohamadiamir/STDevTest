...
class ImagePickerManager: NSObject, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func pickImage(_ viewController: UIViewController) {
        self.sourceVC = viewController
        alert.popoverPresentationController?.sourceView = self.sourceVC!.view
    ...
    private func openCamera(){
        ...
        if(UIImagePickerController .isSourceTypeAvailable(.camera)){
            ...
            self.sourceVC!.present(picker, animated: true, completion: nil)
        } else {
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        ...
        delegate?.didReceiveImage(image: image)
    }
}
